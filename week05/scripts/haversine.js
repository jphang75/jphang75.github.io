function haversine(let1, lon1, let2, lon2) {
    let R = 6371; //earth radius in KM
    let dLat = (let2-let1).toRad();
    let dLon = (lon2-lon1).toRad();
    lat1 = lat1.toRad();
    lat2 = lat2.toRad();

    let a = Math.sin(dLat/2) + Math.sin(dLon2) * Math.sin(dLon2) * Math.cos(lat1) * Math.cos(lat2);
...
let elLocate = document.getElementById("locate");
    elLocate.addEventListener("click", function (){
        ...
        elKlcc.innerHTML = "Distance to De Klcc is " + distances[2];
